...
An honest assessment of what stands between Taktra today — a single-user agent running on
one laptop — and a product other people can sign up for.
...
- Per-user cost controls. Model spend scales with users, and one heavy user can cost more
  than they pay.
...
- Whether each user brings their own model API key or the product pays.
